...
1. Locate iRead by running only `../../scripts/iread workspace` from this skill directory. Installation already performed the environment Doctor check; do not repeat it during normal use. The installer records the repository root; if the wrapper still cannot find it, report the installer error instead of browsing the repository. If an existing subscription has the requested domains or an incomplete activation, show it and ask whether to resume it before creating a duplicate. Run `../../scripts/iread capabilities` once immediately before the first state-changing command, not during initial discovery.
